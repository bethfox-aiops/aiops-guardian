...
**`windows/guardian_process_attribution.ps1` (added 2026-08-26, deployed to both hosts 2026-09-10):** Milestone 3 of the Guardian Edge architecture (`EDGE_ARCHITECTURE.md`) — the M2 gap analysis (2026-08-14) found process-level visibility as the single biggest gap between `windows_exporter`'s enabled collectors and Guardian Core's real checks, since the built-in `process` collector isn't enabled and Behavioral Attestation Phase 1 ("who caused this anomaly") has zero Windows equivalent. Same textfile-collector pattern as `guardian_disk_health.ps1` above rather than just enabling the built-in `process` collector wholesale — that would expose every process, a much wider surface than the curated top-N-suspects pattern `process_attribution.py` already uses on Linux. Writes `windows_top_process_cpu_percent{pid,name}` and `windows_top_process_mem_percent{pid,name}` (top 5 each, via `Get-Counter '\Process(*)\% Processor Time'`/`\ID Process`/`\Working Set` — a formatted/already-rate-calculated counter, so unlike raw WMI this needs no manual before/after delta sampling) plus the same `_collector_last_run_timestamp_seconds` freshness gauge convention.

Deployment is the same shape as `guardian_disk_health.ps1` above (drop the script, register a 15-minute Scheduled Task) — **prerequisite:** the target host's `windows_exporter` needs `textfile` enabled and its textfile directory pointed at the same `textfile_inputs` folder. 15-minute cadence is a known, accepted limitation for anything CPU/mem-transient (a spike between runs is missed) — deliberately not tightened yet, matching M3's own scope ("write exactly one endpoint capability," not a live watchdog).

**Deployed to both Windows hosts (2026-09-10).** `DESKTOP-0AJUKU3` already had the `textfile` collector enabled via the CLI-flag form (`sc.exe config ... --collectors.enabled=...,textfile --collector.textfile.directory=...`). `DESKTOP-503POVP` needed a different path: its `windows_exporter` service was already running with `--config.file="C:\Program Files\windows_exporter\config.yaml"`, so enabling `textfile` there meant editing that YAML file instead of touching `binPath` — and the YAML schema differs from the CLI flag name: `collector.textfile.directories` (plural, a list of paths), not `directory` (singular, a string), confirmed against the [official windows_exporter config.yaml](https://github.com/prometheus-community/windows_exporter/blob/master/config.yaml). Both hosts confirmed live via `windows_disk_health_collector_last_run_timestamp_seconds` and `windows_process_attribution_collector_last_run_timestamp_seconds` in Guardian Core's Prometheus. Still no dashboard panel for the process-attribution metrics specifically.

**Remote access gap found and fixed during this deployment:** `DESKTOP-503POVP` had no remote-admin path at all — it's Windows Home (RDP host mode isn't available on Home, only Pro/Enterprise) and wasn't SSH-reachable. Fixed by installing the OpenSSH Server optional feature (this one *is* available on Home), key-based auth via `C:\ProgramData\ssh\administrators_authorized_keys` (required for local-admin accounts specifically — a normal per-user `authorized_keys` is silently ignored for them), firewall rule scoped to the LAN subnet, and `PasswordAuthentication no` set only after confirming key-based login worked. One gotcha along the way: the firewall rule was correctly scoped but never matched, because Windows had categorized the WiFi connection as `Public` rather than `Private` — reclassifying it (`Set-NetConnectionProfile -NetworkCategory Private`) fixed it, since a `Public`-scoped-out rule silently drops the connection with no error indicating why.
