...
**`EdgePipelineStale` (added 2026-07-30):** `absent_over_time(up{edge_site="guardian-proto-1"}[30m])` for `5m`, severity `high` — pages if the Pi's `remote_write` to Core stops delivering (this exact failure mode ran silently for ~12 days in August before an mDNS-hostname fix).

**AI-risk rules (added 2026-09-10, group `aiops_ai_risk_alerts`):** six rules covering the `guardian_ai_risk.py` rework (Chapter 4/7 detail) — `AIRiskCollectionFailing` (a sub-check has been failing to complete, not just reading clean), `AITrainingDataTampered`, `AIRiskScoreLow` (`<=55` for 15m — recalibrated after the scoring rework so a dev session's own outbound LLM connections, capped at -20, don't false-page at the new lower baseline), `AIExposedApiKeysOnDisk`, `AIShadowModelFiles`, `AICheckStale` (a sub-check hasn't succeeded in 30+ minutes — the same "distinguish failed from clean" principle as `AIRiskCollectionFailing`, one level more specific).
...
**Second, much larger incident, found 2026-09-20 — most notifications were silently failing.** `alertmanager_notifications_failed_total{integration="slack",reason="other"}` showed 234 of 367 send attempts (64%) had failed since the service started on 2026-08-26, with nothing in the journal explaining why — a plain `curl` to the same webhook URL succeeded cleanly, so it wasn't the URL. Root cause, confirmed via `/api/v2/alerts/groups`: the receiver's `title`/`text` templates used `.CommonAnnotations.summary`/`.description`, but `route.group_by` is `['alertname', 'job']` and most alert descriptions render per-instance text (e.g. `"Windows host {{ $labels.instance }} is unreachable"`). Whenever a group holds more than one alert with different rendered text — confirmed live: `WindowsHostUnreachable` was grouped as 4 alerts across `DESKTOP-0AJUKU3` and `DESKTOP-503POVP`, 2 distinct summaries — `.CommonAnnotations` has nothing in common across them and renders empty, and Slack's Incoming Webhook API rejects an empty message body. **This is the actual reason DESKTOP-0AJUKU3's 11-hour outage on 2026-09-15 (Chapter 10) never reached Slack** — the pipeline looked fully wired up and had looked wired up since August, but wasn't reliably working the whole time.

Fixed the same day: `title`/`text` now `range .Alerts` instead, rendering each alert's own label (instance, or check, falling back to alertname) and its own summary/description — correct regardless of group size. `group_by` is unchanged; batching related alerts into one message is still correct behavior, the template just needed to handle more than one alert per message. Verified by injecting a synthetic 2-instance group mirroring the exact failing shape via `amtool alert add`: `alertmanager_notifications_total` incremented while `notifications_failed_total{reason="other"}` stayed flat. Reloaded live via `curl -X POST localhost:9093/-/reload` — no restart needed, no downtime.
...
- Alerting covers the three ML watchdogs' sustained-anomaly case, Windows host unreachability (2026-07-29), edge pipeline staleness (2026-07-30), and the six AI-risk rules (2026-09-10). Guardian Health/Security scores and the Logs watchdog still don't have their own Prometheus alerting rules (they do feed into the Priority watchdog's triage score, Chapter 4.11, but that's a dashboard signal, not a push notification).
- **The gap no rule can close: Core itself going dark.** Every rule above depends on Prometheus and Alertmanager actually running. See 8.6 below — this host suspended for ~65 hours in September and nothing paged anyone, because nothing was running to page anyone. There is no external, always-on watcher of Guardian's own uptime today.
...
### 8.5 Silences for Known Maintenance Windows

`amtool silence add` (or the Alertmanager web UI at `:9093`) suppresses *notifications* for a matched, time-bounded window without touching the underlying rule — the alert still evaluates, still shows as firing in Prometheus/Grafana/the Priority watchdog, only the Slack ping is held back. The right tool for "I know this is expected to be down for a known reason," wrong tool would be editing the rules file (needs sudo, easy to forget to revert). Real example, 2026-09-20: both Windows hosts silenced for 7 days while at a repair shop —

```
amtool silence add 'alertname="WindowsHostUnreachable"' \
  'instance=~"DESKTOP-0AJUKU3:9182|DESKTOP-503POVP:9182"' \
  --duration=7d --comment="<why>" --author=beth \
  --alertmanager.url=http://localhost:9093
```

Scope matchers narrowly (specific instances, not the whole alertname) so an unrelated host added during the window still pages normally. No cleanup needed — the silence simply stops mattering once it expires; extend before it lapses if the window runs long.

### 8.6 Recent Incidents (2026-09-10 through 2026-09-20)

**Core host suspended for ~65 hours, 2026-09-11 22:14 UTC → 2026-09-14 22:04 UTC.** Not a crash — `journalctl` shows a clean `PM: suspend entry (s2idle)` / `PM: suspend exit` pair, gnome-settings-daemon's default `sleep-inactive-ac-type=suspend` at an 8-hour idle timeout, even on AC power. For the entire window, Prometheus, Alertmanager, and every watchdog were simply not running. This was discovered only because a Windows host (below) happened to look down right as the host woke up, and Prometheus's own query history turned out to have a hard cutoff at the wake time — not a TSDB bug, genuinely no data, because nothing was collecting it. **Recommended fix, not yet applied** (checked 2026-09-20 via `gsettings get`, still `sleep-inactive-ac-type='suspend'`, 8h timeout): `gsettings set org.gnome.settings-daemon.plugins.power sleep-inactive-ac-type 'nothing'` on this host specifically, since it also runs the monitoring stack. No fix yet for detecting a recurrence from outside either — see 8.3's last bullet. Two real open items from one incident, not one.

**`DESKTOP-0AJUKU3` crashed twice within 24 hours, 2026-09-14/15** — a third recurrence of the power-quality pattern tracked since 2026-07-29 (Chapter 10). First hang ~22:04 UTC 9/14 (user reset it, recovered); second hang ~04:31 UTC 9/15, undetected for ~11 hours (the Alertmanager bug above is why). `guardian_disk_health.ps1` (Chapter 4.9) caught real new evidence this time: Event ID 153 (I/O device error) logged in the 24h before the second crash, and disk busy% spiked from ~0.5% to 32% in the collector's last sample before the host went unresponsive. Physical recovery needed a hard power cycle, which surfaced a CMOS checksum error and then "no boot device" — diagnosed as a boot-configuration issue (BIOS storage-controller mode reset by the CMOS clear), not drive failure: HP's own hardware diagnostics passed the drive on both Quick and Extensive tests, and it was visible in BIOS storage configuration, just missing from the boot device list. Machine taken to a repair shop rather than resolved remotely.

**`DESKTOP-503POVP` compromised via a tech-support scam, 2026-09-18.** A caller impersonating Best Buy support talked the user into granting RDP access, then showed a fake "Trojan Spyware Detected" scareware screen (identified as fake specifically because the "commands" it told the user to type into `cmd.exe` weren't commands at all — plain scare-copy, `'Trojan' is not recognized as an internal or external command`). Session ended before any financial/account access occurred. Manual inspection afterward (no Guardian tooling involved — this is entirely outside what Guardian can see on a Windows host today) found `UltraViewer` installed the same day as the real access vector, and — the significant find — a scheduled task named `SoftLandingCreativeManagementTask` in `\Softlanding\<user-SID>\`, mimicking the name of a legitimate Microsoft in-box task (`SoftLandingTask`, signed, lives under `\Microsoft\Windows\...`) closely enough to hide in a 123-task list. Confirmed malicious by three details together: exact name match to a documented COM-hijacking persistence technique, non-Microsoft path, and — critically — it had run again that same morning, meaning it was still active on every boot, independent of the original scam call. No local user accounts were added. Machine taken to the repair shop rather than remediated at home, given the sophistication (COM hijacking, documented credential-targeting behavior) — this exceeded what a home checklist should be trusted to fully clear.

**What this exposed, plainly:** Guardian has zero visibility into any of the actual detection work that caught the `DESKTOP-503POVP` compromise — no Windows Event Log collection, no antivirus/EDR telemetry, nothing beyond `windows_exporter`'s performance counters (Chapter 4.9). Every step of that investigation (Task Scheduler inspection, BIOS diagnostics, malware identification) was manual, human-driven, and outside anything this project currently collects. The TODO item for Windows Event Log collection (extending the existing textfile-collector pattern `guardian_disk_health.ps1` already uses, watching for Critical/Error System-log entries) has real, fresh motivating evidence now, not just the original DESKTOP-0AJUKU3 precedent.

### 8.7 Summary

Alerting went from nonexistent to a working Prometheus→Alertmanager→Slack pipeline in one session back in July — but "verified against a real alert" and "reliably works" turned out to be different claims: the pipeline silently dropped 64% of Slack sends for three weeks (8.2) before being caught and fixed. Combined with the Core-suspend blackout (8.6), the honest summary as of 2026-09-20 is narrower than it was: the alerting *mechanism* works now, verified two ways (a real alert and a synthetic reproduction), but the system's ability to notice its own downtime — both Alertmanager's delivery and Core's own uptime — has already failed silently once each, and only one of those two failure modes has anything watching for a recurrence.
...
- **`ai_risk_score` structurally pinned below 100, plus five weaker AI-risk sub-checks, reworked (2026-09-10/11).** Found by reviewing the "AI Anomaly Detection" Grafana dashboard against `guardian_ai_risk.py`: installed AI packages and Guardian's *own* watchdog processes each cost -10 unconditionally, so a clean host could never score above 80 — the dashboard's "green" threshold had been quietly lowered to 80 to compensate rather than the root cause fixed. Reworked: installed packages now informational only; `detect_ai_processes()` excludes Guardian's own processes; penalties scaled with count and capped instead of flat; new `ai_risk_collection_ok`/`ai_check_last_success_timestamp{check}` distinguish a sub-check that raised from one that ran clean; `ai_risk_reason{reason=<key>}` explains which factor is deducting how many points; `get_outbound_llm_connections()` switched from reverse-DNS (silently broken behind Cloudflare) to forward-resolving known LLM API hosts, now correctly detecting real traffic the old check missed entirely; `get_exposed_api_keys()` broadened beyond 5 hardcoded paths. Dashboard re-pushed to match (thresholds fixed, score-history/reasons/collection-status panels added, two stale hardcoded-text panels removed once they started contradicting the query-driven tiles). Verified: 31 tests passing (was 22), smoke-tested against the live host before and after restart.
- **Integrity hashes upgraded from MD5 to SHA-256; systemd-unit check fixed to hash content, not filename+mtime (2026-09-11).** Two follow-ups from reviewing the AI-risk rework above. MD5's broken collision resistance meant a motivated attacker could in principle craft a modified file hashing identically to the original — SSH keys, cron, model files, and the training-data tamper check all hashed with MD5 until this. Separately, `_hash_systemd_units()` hashed filenames+mtimes only, meaning a `.service` file edit could survive undetected if the mtime was reset (`touch -d`, or any tool restoring original timestamps) — fixed to hash actual content. Neither changes what any check exports (same 0/1 flags), just how forgeable the check is.
- **Slack notifications silently failing on grouped alerts, found and fixed (2026-09-20)** — see Chapter 8.2 for full detail. 64% of Slack sends had been failing since Alertmanager started (2026-08-26), unnoticed until a Windows host's 11-hour outage never reached Slack.
...
- **No external watcher of Core's own uptime, real consequence 2026-09-14.** This host suspended for ~65 hours; Prometheus/Alertmanager/every watchdog were simply not running the whole time, undetected — see Chapter 8.3/8.6. Recommended fix for the immediate trigger (disable auto-suspend on this host specifically) has been proposed but **not applied** — checked 2026-09-20, `sleep-inactive-ac-type` is still `'suspend'`. No fix at all yet for the harder problem: something outside this host noticing if it goes dark again.
- **No Windows Event Log collection — real cost, not theoretical.** `guardian_disk_health.ps1` (Chapter 4.9) covers 5 disk-specific Event IDs only; nothing collects general System/Security log content the way Promtail does for Linux journals. Made concrete 2026-09-18: diagnosing a real compromise on `DESKTOP-503POVP` (Chapter 8.6) — a malicious scheduled task mimicking a legitimate Microsoft component — was entirely manual, entirely outside anything Guardian collects. A proposed extension (a scheduled PowerShell script watching for Critical/Error entries and known BugCheck/unexpected-shutdown event IDs, feeding the existing textfile-collector pattern) has real motivating evidence now from two separate hosts, still not built.
