...
Unchanged categories from April (port conflicts, script exits immediately, missing service unit, DHCP breaking Kubernetes certs) — plus, new: **ML watchdog sustained-anomaly drift** is now a recognized recurring pattern with (as of 2026-08-05) three distinct known triggers, not a fresh incident each time — see Chapter 10 before re-diagnosing from scratch: KNN's promtail-write-rate pattern (4 recurrences, 2026-07-13 through -27), and two newer triggers found 2026-08-03 that can hit KNN *and* the autoencoder together — a full reboot, and (milder, sometimes self-resolving) a suspend/resume cycle. Check `journalctl -k` for `PM: suspend exit` at the anomaly-onset time before assuming it's the promtail pattern.
...
- **No process check that sibling retrain scripts stay consistent** — the `RECENT_ROWS` bug itself is fixed in all three scripts now (10.1), but nothing would catch the *next* time one script's constant drifts from its siblings; this is a process gap, not just the one bug.
- **KNN sustained-anomaly drift, promtail-write-rate pattern, still at 4 recurrences** (2026-07-13, -16, -17, -27) — no 5th occurrence of *this specific* signature as of this revision. Retraining has worked as a stopgap every time; if it recurs a 5th time, the right fix is probably structural (a training window that better captures promtail's write-burst variability) rather than "retrain again." **Separately, two more drift trigger categories were found this week** (reboot-triggered and suspend/resume-triggered, both 2026-08-03, see 10.1) — different root cause, same "check labels after any host state change, don't assume healthy" lesson. Given this laptop suspends/resumes multiple times a day, expect that trigger to recur more often than the promtail pattern.
