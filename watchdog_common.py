...
Shared implementation for the aiops-watchdog-{knn,iforest,autoencoder}.py
services: metric collection, GPU handling, Prometheus gauge definitions, and
the collect/score/attribute loop. Each watchdog script supplies a
WatchdogConfig with its model-specific load/build-input/score logic and
calls run(config); everything else (disk/cpu/mem/net/GPU collection,
Behavioral Attestation attribution, logging, shutdown) is identical across
the three and lives here.
"""
...
def require_file(path: str, trainer_name: str) -> None:
    if not os.path.exists(path):
        print(f"[ERROR] Missing {path}. Train model first with {trainer_name}", flush=True)
        sys.exit(1)
...
class WatchdogConfig:
    model_name: str  # e.g. "KNN" / "IFOREST" / "AUTOENCODER" -- used in gauge help text
    default_port: str
    load_model: Callable[[], dict[str, Any]]  # -> state dict, must include "scaler"
    build_input: Callable[[list[float], list[str]], Any]  # (features, DATA_FEATURES) -> X
    score: Callable[[dict[str, Any], Any], tuple[int, float]]  # (state, X_scaled) -> (label, score)


def _make_gauges(model_name: str):
    gauges = {}

    gauges["disk_usage_percent"] = Gauge(
        "aiops_disk_usage_percent", "Disk usage percent of root filesystem"
    )
    gauges["disk_free_gb"] = Gauge(
        "aiops_disk_free_gb", "Free disk space on / in gigabytes"
    )
    gauges["disk_fill_rate_mb_min"] = Gauge(
        "aiops_disk_fill_rate_mb_min",
        "Disk usage fill rate on / in MB per minute (positive means filling)"
    )
    gauges["inode_usage_percent"] = Gauge(
        "aiops_inode_usage_percent",
        "Inode usage percent on /"
    )
    gauges["cpu_usage_percent"] = Gauge(
        "aiops_cpu_usage_percent", "CPU usage percent"
    )
    gauges["mem_usage_percent"] = Gauge(
        "aiops_mem_usage_percent", "Memory usage percent"
    )
    gauges["net_kbps"] = Gauge(
        "aiops_net_kbps", "Total network throughput (send+recv) in kB/s"
    )
    gauges["disk_write_kbps"] = Gauge(
        "aiops_disk_write_kbps", "Disk write throughput in kB/s"
    )
    gauges["gpu_util_percent"] = Gauge(
        "aiops_gpu_util_percent", "GPU utilization percent"
    )
    gauges["gpu_mem_mib"] = Gauge(
        "aiops_gpu_mem_mib", "GPU memory used in MiB"
    )
    gauges["gpu_temp_c"] = Gauge(
        "aiops_gpu_temp_c", "GPU temperature in Celsius"
    )

    gauges["anomaly_label"] = Gauge(
        "aiops_anomaly_label",
        f"Anomaly label from {model_name} (0=normal, 1=anomaly)",
    )
    gauges["anomaly_score"] = Gauge(
        "aiops_anomaly_score",
        f"Anomaly score from {model_name} decision_function (higher=more normal, lower=more anomalous)",
    )

    # Legacy metric for compatibility with existing dashboards/alerts
    gauges["disk_anomaly_prediction"] = Gauge(
    ...
    gauges["suspect_syscall_count"] = Gauge(
    ...
    gauges["top_process_gpu_mem_mib"] = Gauge(
...
def run(config: WatchdogConfig) -> None:
    port = int(os.getenv("WATCHDOG_PORT", config.default_port))
    ...
    state = config.load_model()
    gpu_handle = init_gpu(gpu_index)

    g = _make_gauges(config.model_name)
    ...
    try:
        ...
        while True:
            ...
            g["disk_free_gb"].set(disk_free_gb)
            ...
            g["disk_anomaly_prediction"].set(label)
            ...
            if label == 1:
                ...
                if top_procs:
                    suspect = top_procs[0]
                    g["top_process_info"].clear()
                    g["top_process_info"].labels(
                        pid=str(suspect["pid"]), name=suspect["name"]
                    ).set(1)
                    g["top_process_cpu_percent"].set(suspect["cpu_percent"])
                    g["top_process_mem_percent"].set(suspect["mem_percent"])
                    print(
                    ...
                    if evidence:
                        g["suspect_syscall_count"].clear()
                        for syscall_type, count in evidence["counts"].items():
                            g["suspect_syscall_count"].labels(
                    ...
                    g["top_process_gpu_mem_mib"].set(gpu_mem)
                    if gpu_mem > 0:
